...
public class TimeUtil {
	...
	public static String timeAgoFormat(String timeStr) {
		Date date;
		try {
	...
	public static String timeAgo(String timeStr) {
		System.out.println("timedate"+timeStr);
		if(timeStr==null){
			return " ";
	...
	public static String timeLeft(String timeStr) {
		...
		long hours = Math
				.abs((total_seconds - days * 24 * 60 * 60) / (60 * 60));
		long minutes = Math
				.abs((total_seconds - days * 24 * 60 * 60 - hours * 60 * 60) / 60);
		long seconds = Math.abs((total_seconds - days * 24 * 60 * 60 - hours
				* 60 * 60 - minutes * 60));
		String leftTime;
		if (days > 0) {
			leftTime = days + "天" + hours + "小时" + minutes + "分" + seconds
					+ "秒";
		} else if (hours > 0) {
